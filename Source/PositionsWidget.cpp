    ...
    m_menu(new QMenu(this))
{
    ...
    QAction *actionUpdateItem = new QAction("Изменить позицию", this);
    connect(actionUpdateItem, SIGNAL(triggered()), this, SLOT(on_actionItemUpdate_triggered()));
    m_menu->addAction(actionUpdateItem);

    QAction *actionDeleteItem = new QAction("Удалить позицию", this);
...
void Positions::callContextMenu(const QPoint &position)
{
...
void Positions::on_pushButtonItemInsert_clicked()
{
    if (insertItem())
    {
        m_ui->tableView->setModel(selectData());
        setStatus("Позиция добавлена успешно.");
    }

    on_pushButtonItemUpdateCancel_clicked();
}

void Positions::on_pushButtonItemUpdate_clicked()
{
    if (updateItem())
    {
        m_ui->tableView->setModel(selectData());
        setStatus("Позиция обновлена успешно.");
    }

    on_pushButtonItemUpdateCancel_clicked();
}

void Positions::on_pushButtonItemUpdateCancel_clicked()
{
    m_ui->spinBoxItemRow->clear();
    m_ui->spinBoxItemRack->clear();
    m_ui->pushButtonItemUpdate->hide();
    m_ui->pushButtonItemUpdateCancel->hide();
...
void Positions::on_actionItemUpdate_triggered()
{
    if (QSqlQuery *result = selectItem())
    {
        ...
        m_ui->spinBoxItemRow->setValue(result->value(1).toInt());
        m_ui->spinBoxItemRack->setValue(result->value(2).toInt());
...
void Positions::on_actionItemDelete_triggered()
{
    if (deleteItem())
    {
        m_ui->tableView->setModel(selectData());
        setStatus("Позиция удалена успешно.");
    }
}
